...
| D004 | M002/S02/T03 | testing | Seed data deterministic values documented as code constants | Query actual seed database and record discovered values as typed constants in tests/common/mod.rs | Swiss Ephemeris produces deterministic output for given coordinates, so querying the loaded database yields stable values that integration tests can assert against. This avoids hardcoding astronomical calculations and ensures tests match actual database content. | Yes | agent |
| D005 | M003 | api | Cursor encoding format | Opaque base64-encoded JSON containing (created_at, id) tuple | Base64 opacity allows the wire format to evolve (e.g., adding filter state) without breaking clients. Clients treat cursors as opaque tokens. | Yes — if clients need readable cursors for debugging | agent |
| D006 | M003 | api | Pagination model | Filters + cursor model — first request accepts filter parameters and count, response includes next/prev URLs with encoded cursors that preserve filter state | Keeps the API explorable — clients can construct queries from scratch without a session. Cursors preserve filter state automatically in next/prev URLs. | No | agent |
| D007 | M003 | api | DELETE endpoint scope and in_process guard | Single-job DELETE only; returns 409 Conflict for in_process jobs to prevent orphaning active workers | User chose to exclude in_process jobs from deletion to prevent orphaning workers. Bulk delete deferred to keep scope focused. | Yes — bulk delete may be added in a future milestone | collaborative |
